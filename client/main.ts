...
window.addEventListener("message", (e) => {
    ...
    if (e.data.source === "decoratorClient" && e.data.event == "params") {
        if (e.data.payload.breadcrumbs) {
            const breadcrumbsWrapperEl = document.getElementById("breadcrumbs-wrapper");
...
for (const contextLink of contextLinks) {
    contextLink.addEventListener("click", (e) => {
        ...
        const formatted = formatParams(newParams);

        console.log(newParams, formatted)
...
function attachAmplitudeLinks() {
    ...
    document.body.addEventListener("click", (e) => {
        ...
        if ((e.target as Element).parentNode?.classList.contains("amplitude-link")) {
            alert("Found an ampltidude link")
        }
    });
}
