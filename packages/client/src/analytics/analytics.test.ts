...
const setupDecoratorData = () => {
	...
	(window.__DECORATOR_DATA__.params as Record<string, unknown>).decoratorModulerAnalyticsEntryPoint = 'typed';
};
...
describe('analytics', () => {
	...
	it('uses the decorator origin when the app has not configured one', () => {
		delete (window.__DECORATOR_DATA__.params as Partial<typeof window.__DECORATOR_DATA__.params>).origin;

		initAnalytics(auth);
		vi.advanceTimersByTime(100);

		expect(logUmamiEvent).toHaveBeenCalledWith('besøk', expect.any(Object), 'nav-dekoratoren', expect.any(Object));
	});
	...
	it.each(['typed', 'custom', 'legacy'] as const)(
		...
		async (analyticsEntryPoint) => {
			...
			expect(logUmamiEvent).toHaveBeenCalledWith('test-event', {}, 'test-app', {
				decoratorModulerAnalyticsEntryPoint: analyticsEntryPoint,
			});
		}
	);
	...
	it('ignores invalid analytics entry point without dropping the app event', async () => {
		...
		expect(logUmamiEvent).toHaveBeenCalledWith('test-event', {}, 'test-app', undefined);
	});
});
