...
const stubNavigator = (userAgent: string, extra: Record<string, unknown> = {}) => {
...
describe('normalizeVersion', () => {
	...
	it('collapses the two Chrome reporting paths into one bucket', () => {
		...
		expect(normalizeVersion('151')).toBe(normalizeVersion('151.0.7922.112'));
	});
...
describe('legacy user agent parsing', () => {
	it.each(userAgents)('parses %s', (_label, userAgent, maxTouchPoints, expected) => {
		...
		expect({
			os,
			osVersion: normalizeVersion(getOSVersionFromLegacyUA(userAgent, os)),
...
describe('getBrowserFromBrands', () => {
	...
	it.each([
		...
		expect(getBrowserFromBrands([{ brand, version }, chromium, chrome])).toEqual({ name: 'Chrome', version: '151' });
	});
	...
	it.each([
		...
		expect(
			getBrowserFromBrands([{ brand: 'Not:A-Brand', version: '24' }, chromium, { brand, version: '151' }])
		).toEqual({ name: expected, version: '151' });
	});

	it('falls back to Chromium when no specific brand is present', () => {
		expect(getBrowserFromBrands([{ brand: 'Not:A-Brand', version: '24' }, chromium])).toEqual({
			name: 'Chrome',
			version: '151',
		});
	});
	...
	it("does not treat a real brand containing 'Not' as GREASE", () => {
		expect(getBrowserFromBrands([{ brand: 'Notion Browser', version: '3' }, chromium])).toEqual({
			name: 'Notion Browser',
			version: '3',
		});
	});
