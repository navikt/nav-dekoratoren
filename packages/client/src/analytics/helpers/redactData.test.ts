...
describe('redactData', () => {
	...
	describe('string values', () => {
		...
		it('should redact UUIDs in strings', () => {
			...
			expect(redactData(`some text ${uuid} more text`)).toBe('some text [redacted: uuid] more text');
		});

		it('should redact multiple UUIDs in a string', () => {
			const uuid1 = '123e4567-e89b-12d3-a456-426614174000';
			const uuid2 = '987fcdeb-51a2-3bc4-d567-890123456789';
			expect(redactData(`${uuid1} and ${uuid2}`)).toBe('[redacted: uuid] and [redacted: uuid]');
		});

		it.each(['url', 'referrer', 'destinasjon'])('should apply URL redaction for %s key', (key) => {
			setRedactPaths([['/person/:redact:/sak', { redactPath: true, redactTitle: false }]]);

			expect(redactData('/person/12345/sak', key)).toBe('/person/[redacted]/sak');
		});

		it('should not apply URL redaction for non-URL keys', () => {
			setRedactPaths([['/person/:redact:/sak', { redactPath: true, redactTitle: false }]]);

			expect(redactData('/person/12345/sak', 'someOtherKey')).toBe('/person/12345/sak');
		});
		...
		it('should redact both URL path and UUIDs when applicable', () => {
			...
			const uuid = '123e4567-e89b-12d3-a456-426614174000';
			expect(redactData(`/person/${uuid}/sak`, 'url')).toBe('/person/[redacted]/sak');
		});
	});
	...
	describe('local path redaction', () => {
		describe('Unix/macOS paths', () => {
			it('should redact /users/ (lowercase)', () => {
				expect(redactData('/users/john/documents/file.txt', 'url')).toBe('[redacted: local path]');
			});
			...
			it('should redact /home/ paths (Linux)', () => {
				expect(redactData('/home/john/documents/file.txt', 'url')).toBe('[redacted: local path]');
			});

			it('should redact file:// protocol with Unix path', () => {
				expect(redactData('file:///Users/dev/project/index.html', 'referrer')).toBe('[redacted: local path]');
			});

			it('should redact file:// protocol with Linux path', () => {
				expect(redactData('file:///home/dev/project/index.html', 'url')).toBe('[redacted: local path]');
			});
		});

		describe('Windows paths', () => {
			it('should redact Windows path with backslashes', () => {
				expect(redactData('C:\\Users\\john\\Documents\\file.pdf', 'url')).toBe('[redacted: local path]');
			});
			...
			it('should redact file:// protocol with Windows path', () => {
				expect(redactData('file:///C:/Users/john/Documents/file.pdf', 'url')).toBe('[redacted: local path]');
			});

			it('should redact various drive letters', () => {
				expect(redactData('D:\\data\\file.txt', 'url')).toBe('[redacted: local path]');
				expect(redactData('E:/backup/file.txt', 'url')).toBe('[redacted: local path]');
			});

			it('should redact Windows path with leading slash', () => {
				expect(redactData('/C:/Users/Users Name/Desktop/soknader Ortoser.htm', 'url')).toBe('[redacted: local path]');
			});
		});

		describe('URL key handling', () => {
			it('should redact for destinasjon key', () => {
				expect(redactData('/users/someone/downloads/report.pdf', 'destinasjon')).toBe('[redacted: local path]');
			});

			it('should not redact local paths for non-URL keys', () => {
				expect(redactData('/users/john/file.txt', 'someOtherKey')).toBe('/users/john/file.txt');
			});
		...
		describe('should NOT redact legitimate web URLs', () => {
			it('should not redact /api/users/ paths', () => {
				expect(redactData('/api/users/123', 'url')).toBe('/api/users/123');
			});

			it('should not redact /api/home/ paths', () => {
				expect(redactData('/settings/home/dashboard', 'url')).toBe('/settings/home/dashboard');
			});

			it('should not redact full URLs with /users/ in path', () => {
				expect(redactData('https://api.example.com/api/users/123', 'url')).toBe(
					'https://api.example.com/api/users/123'
				);
			});

			it('should not redact full URLs with /home/ in path', () => {
				expect(redactData('https://example.com/home/dashboard', 'url')).toBe('https://example.com/home/dashboard');
			});
		});
	});
	...
	describe('array values', () => {
		it('should recursively redact array items', () => {
			const uuid = '123e4567-e89b-12d3-a456-426614174000';
			expect(redactData(['hello', uuid, 'world'])).toEqual(['hello', '[redacted: uuid]', 'world']);
		});

		it('should handle nested arrays', () => {
			const uuid = '123e4567-e89b-12d3-a456-426614174000';
			expect(redactData([['nested', uuid]])).toEqual([['nested', '[redacted: uuid]']]);
		});
	...
	describe('object values', () => {
		...
		it('should apply URL redaction to url key in objects', () => {
			...
			expect(redactData({ url: '/person/12345/sak', name: 'test' })).toEqual({
	...
	describe('title redaction based on URL', () => {
		it('should redact title when URL matches pattern with redactTitle: true', () => {
			setRedactPaths([['/person/:redact:/sak', { redactPath: true, redactTitle: true }]]);
		...
		it('should redact title even when redactPath is false', () => {
			setRedactPaths([['/person/:redact:/sak', { redactPath: false, redactTitle: true }]]);
	...
	describe('uuid redaction opt-out', () => {
		...
		it('should still apply URL path redaction when uuid opt-out is set', () => {
			...
			expect(redactData('/person/12345/sak', 'url')).toBe('/person/[redacted]/sak');
		});
	});
});
