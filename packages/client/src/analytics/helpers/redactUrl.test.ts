...
describe('redactFromUrl', () => {
	...
	const setRedactPaths = (paths: Array<[string, RedactConfig]> | string[]) => {
	...
	describe('when redacting path segments', () => {
		it('should redact matching path segments', () => {
			...
			expect(redactFromUrl('/person/12345678901/sak').redactedUrl).toBe('/person/[redacted]/sak');
		});

		it('should handle multiple :redact: placeholders in a pattern', () => {
			setRedactPaths(['/person/:redact:/dokument/:redact:']);

			expect(redactFromUrl('/person/12345678901/dokument/abc123').redactedUrl).toBe(
		...
		it('should preserve query strings and hash', () => {
			...
			expect(redactFromUrl('/person/12345678901/sak?foo=bar#section').redactedUrl).toBe(
				'/person/[redacted]/sak?foo=bar#section'
			);
		});
	...
	describe('when URL does not match any pattern', () => {
		it('should return the original URL unchanged', () => {
			...
			expect(redactFromUrl('/other/path/here').redactedUrl).toBe('/other/path/here');
		});

		it('should not match when path is shorter than pattern', () => {
			setRedactPaths(['/person/:redact:/sak']);

			expect(redactFromUrl('/person/12345678901').redactedUrl).toBe('/person/12345678901');
		});
	});
	...
	describe('prefix matching', () => {
		it('should match and redact when URL has additional segments after pattern', () => {
			...
			expect(redactFromUrl('/foobar/mypage/382737/list').redactedUrl).toBe('/foobar/mypage/[redacted]/list');
			expect(redactFromUrl('/foobar/mypage/3827273/list/page2').redactedUrl).toBe(
		...
		it('should match exact length paths as well', () => {
			...
			expect(redactFromUrl('/foobar/mypage/382737').redactedUrl).toBe('/foobar/mypage/[redacted]');
		});

		it('should handle multiple :redact: with prefix matching', () => {
			setRedactPaths(['/person/:redact:/dokument/:redact:']);

			expect(redactFromUrl('/person/123/dokument/abc/extra/segments').redactedUrl).toBe(
				'/person/[redacted]/dokument/[redacted]/extra/segments'
			);
		});
		...
		it('should preserve query strings and hash with prefix matching', () => {
			...
			expect(redactFromUrl('/foobar/mypage/382737/list?foo=bar#section').redactedUrl).toBe(
				'/foobar/mypage/[redacted]/list?foo=bar#section'
			);
		});
	});
	...
	describe('with absolute URLs', () => {
		it('should handle absolute URLs correctly', () => {
			...
			expect(redactFromUrl('https://www.nav.no/person/12345678901/sak').redactedUrl).toBe(
				'https://www.nav.no/person/[redacted]/sak'
			);
		});
		...
		it('should handle absolute URLs with query and hash', () => {
			...
			expect(redactFromUrl('https://www.nav.no/person/12345678901/sak?a=b#hash').redactedUrl).toBe(
				'https://www.nav.no/person/[redacted]/sak?a=b#hash'
			);
		});
	});
	...
	describe('edge cases', () => {
		...
		it('should handle empty path patterns gracefully', () => {
			...
			expect(redactFromUrl('/person/12345678901/sak').redactedUrl).toBe('/person/12345678901/sak');
		});

		it('should handle multiple matching patterns', () => {
			setRedactPaths(['/person/:redact:/sak', '/bruker/:redact:/dokument', '/annen/:redact:/liste/:redact:']);

			expect(redactFromUrl('/person/12345678901/sak').redactedUrl).toBe('/person/[redacted]/sak');
			expect(redactFromUrl('/bruker/abc123/dokument').redactedUrl).toBe('/bruker/[redacted]/dokument');
			expect(redactFromUrl('/annen/abc123/liste/xyz789').redactedUrl).toBe('/annen/[redacted]/liste/[redacted]');
		});
	...
	describe('shouldRedactTitle behavior', () => {
		...
		it('should not redact path or title when both are false', () => {
			setRedactPaths([['/person/:redact:/sak', { redactPath: false, redactTitle: false }]]);
	...
	describe('redactPath behavior', () => {
		...
		it('should handle mixed redactPath configs across multiple patterns', () => {
			setRedactPaths([
				['/person/:redact:/sak', { redactPath: true, redactTitle: false }],
				['/bruker/:redact:/info', { redactPath: false, redactTitle: true }],
			]);
