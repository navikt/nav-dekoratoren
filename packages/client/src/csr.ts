...
const findOrError = (id: string) => {
    ...
    if (!el) {
        throw new Error(`Could not find element with id ${id}. See documentation on nav-dekoratoren on github.`);
    }
...
export async function hydrate() {
    const [header, footer, envEl] = ['decorator-header', 'decorator-footer', 'decorator-env'].map(findOrError);
