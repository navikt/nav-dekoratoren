...
describe('decoratorApi across APP_URL shapes', () => {
	...
	const appUrlShapes: Array<[shape: string, appUrl: string, path: string]> = [
		['a single-segment prefix (prod: /dekoratoren)', 'http://localhost/dekoratoren', '/dekoratoren/main-menu'],
		[
		...
		['a trailing-slash APP_URL, without doubling the slash', 'http://localhost/dekoratoren/', '/dekoratoren/main-menu'],
	];

	it.each(appUrlShapes)('resolves the route under %s', async (_shape, appUrl, path) => {
	...
	it('does not resolve root-relative paths the way new URL does', async () => {
		...
		expect(http.lastCall?.url).toContain('https://www.nav.no/dekoratoren/main-menu?');
	});
});
