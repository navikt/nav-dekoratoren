import {
    ...
    makeBoostConfig,
} from "decorator-shared/boost-config";
...
class ChatbotWrapper extends HTMLElement {
    ...
    update() {
        ...
        const scriptLoaded = loadedScripts.has(
            getScriptUrl(env("BOOST_ENVIRONMENT") as BoostEnviroment),
        );
    ...
    showChatbot() {
        const shouldShow =
            window.__DECORATOR_DATA__.params.chatbot &&
            window.__DECORATOR_DATA__.params.chatbotVisible;
...
function getPreferredFilter(): PreferredFilter {
    return param("context") === "arbeidsgiver"
        ? "arbeidsgiver"
        : param("language") === "nn"
          ? "nynorsk"
          : "bokmal";
