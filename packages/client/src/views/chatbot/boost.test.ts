...
describe('init boost', () => {
	...
	const chatPanel = document.createElement('div') as unknown as BoostClient['chatPanel'];
	...
	it('sets preferred filter to arbeidsgiver', async () => {
		...
		initBoost();
		await boostInitialized();
		expect(calledWith[1].chatPanel.header.filters.filterValues).toBe('arbeidsgiver');
	});
	...
	it('sets preferred filter to nynorsk', async () => {
		...
		initBoost();
		await boostInitialized();
		expect(calledWith[1].chatPanel.header.filters.filterValues).toBe('nynorsk');
	});
