...
describe('MainMenu', () => {
	...
	it('fetches and renders menu content on connect', async () => {
		http.get(apiPath('/main-menu'), { text: '<p>meny</p>' });
	...
	it('refetches on a context change', async () => {
		...
		http.get(apiPath('/main-menu'), { text: '<p>arbeidsgiver-meny</p>' });
		dispatchContextUpdated('arbeidsgiver');

		await waitFor(() => expect(el.innerHTML).toBe('<p>arbeidsgiver-meny</p>'));
	...
	it('shows an error message and logs when the fetch fails', async () => {
		...
		http.get(apiPath('/main-menu'), { status: 400 });

		const el = await fixture('<main-menu></main-menu>');

		await waitFor(() => expect(el.innerHTML).toBe('Kunne ikke laste meny-innhold'));
	...
	it('serves a cached menu when a context is revisited', async () => {
		http.get(apiPath('/main-menu'), (call) => ({
			text: `<p>${call.query.get('context')}-meny</p>`,
		}));

		const el = await fixture('<main-menu></main-menu>');
		await waitFor(() => expect(el.innerHTML).toBe('<p>privatperson-meny</p>'));
