...
async function processFile(path: string) {
    ...
    const tokens = val.messages.find(({ type, plugin }) => type === 'export' && plugin === 'postcss-modules')?.exportTokens;
...
function createTSModule(file: ProcessedFile) {
    const tokenProperties = Object.keys(file.tokens).map((token) =>
        ts.factory.createPropertySignature(undefined, token, undefined, ts.factory.createKeywordTypeNode(ts.SyntaxKind.StringKeyword))
    );
    ...
    const classesDeclaration = ts.factory.createVariableStatement(
        undefined,
        ts.factory.createVariableDeclarationList([ts.factory.createVariableDeclaration('classes', undefined, classes, undefined)], ts.NodeFlags.Const)
    );

    const exportClassesStatement = ts.factory.createExportDefault(ts.factory.createIdentifier('classes'));
...
export async function processAll() {
    const paths = getFilePaths(path.resolve(__dirname, '..')).filter((path) => path.endsWith('.module.css'));

    const processedFiles = await Promise.all(paths.map(async (f) => await processFile(f)));
