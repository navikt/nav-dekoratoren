 ...
 */
const inputHash = () => {
	const akselPackage = fileURLToPath(import.meta.resolve('@navikt/aksel-icons/package.json'));
...
const isUpToDate = () => {
	try {
		...
		return manifest.hash === expectedHash && manifest.fileCount === readdirSync(DIST).length;
	} catch {
		return false;
	}
};
...
const akselSvgDir = fileURLToPath(new URL('.', import.meta.resolve(`@navikt/aksel-icons/svg/${akselNames[0]}.svg`)));
...
generated.push(
	{
		file: 'index.ts',
		contents: `export * from "../src";\n${icons.map(({ name }) => `export * from "./${name}";`).join('\n')}\n`,
	},
	{ file: 'types.ts', contents: readUtf8(`${SRC}/types.ts`) }
);

await Promise.all(generated.map(({ file, contents }) => writeFile(`${DIST}/${file}`, contents)));

// Every file this build is responsible for, including the manifest itself.
const expectedFiles = new Set([...generated.map(({ file }) => file), MANIFEST_FILE]);
...
writeFileSync(`${DIST}/${MANIFEST_FILE}`, JSON.stringify({ hash: expectedHash, fileCount: expectedFiles.size }));
