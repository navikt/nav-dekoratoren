...
import { setParams, type DecoratorParams } from '@navikt/nav-dekoratoren-moduler/csr';
...
const formatParams = (params: ReturnType<typeof buildDecoratorParams>) => JSON.stringify(params, null, 2);
...
const getLocalAuthState = () => getCookieValue(localAuthCookieName) as LocalAuthState | undefined;

const getLocalNotificationsState = () =>
	getCookieValue(localNotificationsCookieName) as LocalNotificationsState | undefined;
...
type RouteTargetKey = 'frontpage' | 'arbeidsgiver' | 'person' | 'sak123' | 'sak456';
...
const paramConfigsByLabel = [...paramConfigs].sort((a, b) => a.label.localeCompare(b.label, 'nb'));

const defaultDirectParams: DecoratorParams = {
	language: 'en',
	context: 'samarbeidspartner',
	availableLanguages: paramConfigs.find(({ key }) => key === 'availableLanguages')
		?.defaultValue as DecoratorParams['availableLanguages'],
	breadcrumbs: paramConfigs.find(({ key }) => key === 'breadcrumbs')?.defaultValue as DecoratorParams['breadcrumbs'],
	chatbotVisible: true,
	simple: false,
};

const formatPayload = (params: DecoratorParams) => JSON.stringify(params, null, 2);

const findParamConfig = (key: BuilderParamKey) => paramConfigs.find((config) => config.key === key)!;

const cloneParams = (params: DecoratorParams): DecoratorParams => structuredClone(params);

const isEmptyParams = (params: DecoratorParams) => Object.keys(params).length === 0;
...
const urlParamKeys = ['language', 'context', 'simple', 'chatbotVisible'] satisfies BuilderParamKey[];
...
const getParamConfigsForMode = (mode?: WorkflowMode) => {
	...
	return mode === 'navigation'
		? paramConfigsByLabel.filter(({ key }) => navigationParamKeySet.has(key))
		: paramConfigsByLabel;
};

const getNavigationParams = (params: DecoratorParams) =>
	Object.fromEntries(
		urlParamKeys.filter((key) => params[key] !== undefined).map((key) => [key, params[key]])
	) as DecoratorParams;

const formatPreviewValue = (value: unknown) => JSON.stringify(value);

const buildNavigationHref = (routeTarget: RouteTargetKey, params: DecoratorParams) => {
	const target = routeTargets.find(({ key }) => key === routeTarget)!;
	const searchParams = new URLSearchParams();

	Object.entries(getNavigationParams(params)).forEach(([key, value]) => searchParams.set(key, String(value)));
...
const getAvailableLanguagePreset = (languages: DecoratorParams['availableLanguages']) => {
...
const getAvailableParamConfigs = (params: DecoratorParams, mode?: WorkflowMode) =>
	getParamConfigsForMode(mode).filter(({ key }) => params[key] === undefined);

const getFirstAvailableParamKey = (params: DecoratorParams, mode: WorkflowMode = 'payload') =>
	getAvailableParamConfigs(params, mode)[0]?.key ?? 'language';

export default function ParamBuilder({ initialPath, title }: Props) {
	const router = useRouter();
	const [directParamsUpdate, setDirectParamsUpdate] = useState<DirectParamsUpdate>();
	...
	const [paramToAdd, setParamToAdd] = useState<BuilderParamKey>(() => getFirstAvailableParamKey({}));
	...
	const localAuthState = useSyncExternalStore(emptySubscribe, getLocalAuthState, () => undefined);
	const localNotificationsState = useSyncExternalStore(emptySubscribe, getLocalNotificationsState, () => undefined);
	const routeParams = buildDecoratorParams(path);
	const decoratorParams =
		directParamsUpdate?.path === path ? { ...routeParams, ...directParamsUpdate.params } : routeParams;
	const selectedParamKeys = getParamConfigsForMode(workflowMode)
		.map(({ key }) => key)
		.filter((key) => directParams[key] !== undefined);
	const availableParamConfigs = getAvailableParamConfigs(directParams, workflowMode);
	const selectedParamToAdd = availableParamConfigs.some(({ key }) => key === paramToAdd)
		? paramToAdd
		: getFirstAvailableParamKey(directParams, workflowMode);
	const navigationHref = buildNavigationHref(routeTarget, directParams);
	const selectedRouteTarget = routeTargets.find(({ key }) => key === routeTarget)!;
	const navigationBaseParams = buildDecoratorParams(selectedRouteTarget.pathname);
	const navigationResultParams = buildDecoratorParams(navigationHref);
	const navigationAppliedParamKeys = new Set(Object.keys(getNavigationParams(directParams)));

	const setDirectParam = (key: BuilderParamKey, value: DecoratorParams[BuilderParamKey]) => {
	...
	const removeDirectParam = (key: BuilderParamKey) => {
		...
		setParamToAdd(getFirstAvailableParamKey(nextParams as DecoratorParams, workflowMode));
	};
	...
	const prefillDirectParams = () => {
		const prefilledParams =
			workflowMode === 'navigation' ? getNavigationParams(defaultDirectParams) : defaultDirectParams;
	...
	const emitDirectParams = (channel: 'moduler' | 'postMessage') => {
		...
		const payload = isEmptyParams(params) ? getResetParams(routeParams) : params;
			...
			case 'language':
				return (
					<Select
						...
						onChange={(event) => setDirectParam('language', event.target.value)}
					>
			...
			case 'context':
				return (
					<Select
						...
						onChange={(event) => setDirectParam('context', event.target.value)}
					>
			...
			case 'availableLanguages':
				return (
					<Select
						label="Tilgjengelige språk"
						size="small"
						value={getAvailableLanguagePreset(directParams.availableLanguages)}
						onChange={(event) =>
							setDirectParam(
								'availableLanguages',
								availableLanguagePresets[event.target.value as keyof typeof availableLanguagePresets]
							)
						}
					>
						<option value="none">Ingen</option>
						<option value="nbEn">Bokmål og engelsk</option>
						<option value="nbNnEn">Bokmål, nynorsk og engelsk</option>
					</Select>
				);
			case 'breadcrumbs':
				return (
					<Select
						...
						onChange={(event) =>
							setDirectParam('breadcrumbs', breadcrumbPresets[event.target.value as keyof typeof breadcrumbPresets])
						}
					>
						<option value="none">Ingen</option>
						<option value="person">Person: sykepenger</option>
						<option value="application">Person: søknad om sykepenger</option>
						<option value="employer">Arbeidsgiver: inntektsmelding</option>
					</Select>
				);
			case 'utilsBackground':
				return (
					<Select
						...
						onChange={(event) => setDirectParam('utilsBackground', event.target.value)}
					>
			...
			case 'pageType':
				return (
					<TextField
						...
						onChange={(event) => setDirectParam(key, event.target.value)}
					/>
				);
			default:
				return (
					<Checkbox
						...
						onChange={(event) => setDirectParam(key, event.target.checked)}
					>
			...
			<VStack gap="space-16">
				...
				>
					<VStack gap="space-8">
						<HStack justify="space-between" align="center" gap="space-8" wrap>
							...
							<HStack gap="space-8" wrap>
								...
								<Button
									...
									size="small"
									variant="secondary-neutral"
									disabled={localAuthState === 'logged-in' && localNotificationsState === 'full'}
									onClick={() =>
										applyLocalDecoratorState({
											auth: 'logged-in',
											notifications: 'full',
								...
								<Button
									...
									size="small"
									variant="secondary-neutral"
									disabled={localAuthState === 'logged-in' && localNotificationsState === 'empty'}
									onClick={() =>
										applyLocalDecoratorState({
											auth: 'logged-in',
											notifications: 'empty',
						...
						<pre className={styles.codeBlock}>{formatParams(decoratorParams)}</pre>
					</VStack>
				</Box>
				...
				>
					<VStack gap="space-12">
						...
						<ToggleGroup
							...
							data-color="accent"
							value={workflowMode}
							onChange={(value) => selectWorkflowMode(value as WorkflowMode)}
						>
							<ToggleGroup.Item value="payload" label="Parameteroppdatering" />
							<ToggleGroup.Item value="navigation" label="Navigasjon" />
						</ToggleGroup>
						...
						{workflowMode === 'navigation' && (
							<Select
								...
								value={routeTarget}
								onChange={(event) => setRouteTarget(event.target.value as RouteTargetKey)}
							>
						...
						<HStack gap="space-8" align="end" wrap>
							<Select
								...
								onChange={(event) => setParamToAdd(event.target.value as BuilderParamKey)}
							...
							<Button type="button" size="small" variant="secondary" data-color="neutral" onClick={prefillDirectParams}>
								Fyll inn eksempel
							</Button>
						</HStack>
						...
						{selectedParamKeys.length > 0 ? (
							<Box className={styles.paramGroup}>
								{selectedParamKeys.map((key) => (
									<HStack key={key} align="end" gap="space-8" className={styles.paramRow}>
										<Box className={styles.paramControl}>{renderParamControl(key)}</Box>
										<Button
											type="button"
											size="small"
											variant="primary"
						...
						<Box as="section" aria-labelledby={`${workflowMode}-preview`} className={styles.previewWrapper}>
							<VStack gap="space-8">
								<Heading level="3" size="xsmall" id={`${workflowMode}-preview`}>
									Forhåndsvisning
								</Heading>
								...
								{workflowMode === 'navigation' ? (
									<>
										<pre className={styles.codeBlock}>
											{'{\n'}
											{Object.entries(navigationResultParams).map(([key, value], index, entries) => {
												const appliedParam = navigationAppliedParamKeys.has(key);
												const baseValue = navigationBaseParams[key as BuilderParamKey];
												...
												return (
													<span key={key} className={appliedParam ? undefined : styles.previewRouteValue}>
														{`  "${key}": `}
														{overridesRouteValue ? (
															<>
																<s className={styles.previewOldValue}>{formatPreviewValue(baseValue)}</s> →{' '}
										...
										<BodyShort size="small" className={styles.previewUrl}>
											URL: <code>{navigationHref}</code>
										</BodyShort>
									</>
								) : (
									<pre className={styles.codeBlock}>{formatPayload(directParams)}</pre>
								)}
							</VStack>
						</Box>
						...
						{workflowMode === 'navigation' ? (
							<HStack gap="space-8" wrap>
								<Button as={Link} href={navigationHref} size="small" variant="secondary">
									Naviger med Next Link
								</Button>
								<Button type="button" size="small" variant="secondary" onClick={() => router.push(navigationHref)}>
									Naviger med router.push
								</Button>
								<Button type="button" size="small" variant="secondary" onClick={() => router.replace(navigationHref)}>
									Naviger med router.replace
								</Button>
							</HStack>
						) : (
							<HStack gap="space-8" wrap>
								<Button type="button" size="small" variant="secondary" onClick={() => emitDirectParams('moduler')}>
									Oppdater via moduler
								</Button>
								<Button type="button" size="small" variant="secondary" onClick={() => emitDirectParams('postMessage')}>
