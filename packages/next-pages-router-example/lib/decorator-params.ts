import type { DecoratorLocale, DecoratorParams } from '@navikt/nav-dekoratoren-moduler/ssr';

const validLanguages = ['nb', 'nn', 'en', 'se', 'pl', 'uk', 'ru'] as const;
const validContexts = ['privatperson', 'arbeidsgiver', 'samarbeidspartner'] as const;

const isValidLanguage = (value: string | null): value is DecoratorLocale =>
	validLanguages.includes(value as DecoratorLocale);

const isValidContext = (value: string | null): value is NonNullable<DecoratorParams['context']> =>
...
const chatbotVisibleParam = (url: URL): Pick<DecoratorParams, 'chatbotVisible'> =>
	url.searchParams.has('chatbotVisible') ? { chatbotVisible: url.searchParams.get('chatbotVisible') === 'true' } : {};
