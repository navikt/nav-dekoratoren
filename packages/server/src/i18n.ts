...
export const isNorwegian = (language: Language) => language === 'nb' || language === 'nn';

function i18n(key: keyof Texts, args?: never): Template;
function i18n<Key extends keyof Texts>(
	key: Key,
	args: Texts[Key] extends (...args: any) => any ? Parameters<Texts[Key]>[0] : never
): Template;
	...
	return {
		render: (props: { language: Language } = { language: 'nb' }) => {
			...
			if (typeof text === 'function') {
				...
				return typeof result === 'string' ? result : result.render(props);
			} else {
				return text;
			}
		},
