...
const normalizePath = (url: string) => new URL(url).pathname.replace(/\/+$/, '');
...
describe('ingress path prefixes', () => {
	test('vars files were found and parsed', () => {
		...
		expect(ingressPathsByFile.flatMap(([, paths]) => paths).length).toBeGreaterThan(0);
	});

	test.each(ingressPathsByFile)('every ingress path in %s is mounted', (file, paths) => {
		const missing = paths.filter((path) => !(INGRESS_PATH_PREFIXES as readonly string[]).includes(path));
	...
	test('every mounted prefix is used by an ingress', () => {
		const ingressPaths = new Set(ingressPathsByFile.flatMap(([, paths]) => paths));

		const unused = INGRESS_PATH_PREFIXES.filter((prefix) => prefix !== '/' && !ingressPaths.has(prefix));
