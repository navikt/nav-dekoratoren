...
type OnUpdateCallback<FileContent> = (fileContent: FileContent | null) => unknown;
...
export class ConfigMapWatcher<FileContent extends Record<string, unknown>> {
	...
	constructor({ mountPath, filename, onUpdate, shouldPoll = false }: ConstructorProps<FileContent>) {
		const mountPathFull = path.join(process.cwd(), isLocalhost() ? '/config' : mountPath);
		...
		if (!mountPathStats) {
			logger.error(`Mount path ${mountPathFull} for ${filename} does not exist - configmap file will not be watched`);
			return;
		}
		...
		this.watcher = fs.watch(parentDir, { recursive: false }, (event, fileOrDir) => {
			// Check if the change is related to our mount path
			if (fileOrDir && fileOrDir.includes(path.basename(mountPathFull))) {
				logger.info(`Detected potential ConfigMap update for ${mountPathFull} (${event})`);
				this.checkForUpdate(onUpdate);
			}
		});

		// Also watch the mount path itself for direct file changes
		this.directWatcher = fs.watch(mountPathFull, { recursive: true }, (event, fileOrDir) => {
			if (fileOrDir && path.basename(fileOrDir) !== filename) {
				return;
			}

			logger.info(`Configmap file ${this.filePath} change detected (${event})`);
			this.checkForUpdate(onUpdate);
		});
		...
		const startPolling = () => {
			...
			this.pollTimeout = setTimeout(async () => {
				try {
					...
					logger.error(`Error during polling config map changes for ${this.filePath}`, { error: e });
				} finally {
		...
		logger.info(`Watching for updates on ${mountPathFull} for ${filename} and ${parentDir}.`);
	}
	...
	private async checkForUpdate(onUpdate?: OnUpdateCallback<FileContent>) {
		try {
			...
			if (currentMtime !== this.lastMtime) {
				...
				logger.info(`File ${this.filePath} has been modified (mtime: ${new Date(currentMtime).toISOString()})`);
