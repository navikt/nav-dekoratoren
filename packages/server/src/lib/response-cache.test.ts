...
const setupBackoffState = async (cache: ResponseCache<string>, callback: () => Promise<string | null>) => {
...
describe('ResponseCache with suppressRetryForMs', () => {
	...
	test('returns stale value immediately when within backoff window', async () => {
		const cache = makeCache();
		const callback = vi.fn().mockResolvedValueOnce('stale-data').mockResolvedValue(null);
