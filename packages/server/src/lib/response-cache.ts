...
export class ResponseCache<ValueType = unknown> {
	...
	private readonly pendingPromises = new Map<string, Promise<ValueType | null>>();
	private readonly nextRetryAt = new Map<string, number>();

	constructor({ ttl, suppressRetryForMs = 0, logger }: { ttl: number; suppressRetryForMs?: number; logger: Logger }) {
		this.ttl = ttl;
	...
	async get(key: string, callback: () => Promise<ValueType | null>): Promise<ValueType | null> {
		...
		if (retryAfter && retryAfter > Date.now()) {
			this.logger.warn(`Retry suppressed for key ${key} until ${new Date(retryAfter).toISOString()}`);
	...
	private async getPromise(key: string, callback: () => Promise<ValueType | null>) {
		...
		const promise = callback()
			...
			.catch((e) => {
				this.logger.error(`Callback error while fetching value for key ${key}`, { error: e });
				if (this.suppressRetryForMs > 0) {
					this.nextRetryAt.set(key, Date.now() + this.suppressRetryForMs);
				}
