...
describe('getSimpleFooterLinks', () => {
	...
	beforeAll(() => {
		server = setupServer(http.get(`${env.ENONICXP_SERVICES}/no.nav.navno/menu`, () => HttpResponse.json(testData)));
		server.listen();
	});
	beforeEach(() => {
		vi.useFakeTimers();
	...
	test('urls start with XP_BASE_URL', async () => {
		expect((await getSimpleFooterLinks({ language: 'nb' })).at(0)?.url).toSatisfy((url: string) =>
			url.startsWith(env.XP_BASE_URL)
		);
	});
	...
	test('returns stale menu when Enonic is down', async () => {
		...
		server.use(http.get(`${env.ENONICXP_SERVICES}/no.nav.navno/menu`, () => new HttpResponse(null, { status: 503 })));
	...
	test('does not retry during backoff window after Enonic failure', async () => {
		...
		server.use(
			http.get(`${env.ENONICXP_SERVICES}/no.nav.navno/menu`, () => {
				fetchCount++;
				return fetchCount === 1 ? HttpResponse.json(testData) : new HttpResponse(null, { status: 503 });
			})
		);
