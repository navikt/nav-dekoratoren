...
const fetchMenu = async (): Promise<MainMenu> => {
	const menuFromService = await menuCache.get('menu', () =>
		fetchAndValidateJson(MENU_SERVICE_URL, undefined, mainmenuSchema).then((res) => {
			if (!res.ok) {
...
export const getMainMenuLinks = async ({ language, context }: { language: Language; context: Context }) => {
...
export const getSimpleFooterLinks = async ({ language }: { language: Language }) => {
	const menu = await fetchMenu();

	return get(menu, `${getLangKey(language)}.Footer.Personvern`)?.map(nodeToLink) ?? [];
};
...
const nodeToLinkGroup: (node: MenuNode) => LinkGroup = ({ displayName, children }) => ({
	heading: displayName,
	children: children.map(nodeToLink),
});
...
const nodeToLink: (node: MenuNode) => Link = ({ displayName, path, frontendEventID }) => {
...
const getRecursive = (node: MenuNode | undefined, path: string): MenuNode | undefined => {
	if (path.includes('.')) {
