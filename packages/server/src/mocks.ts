...
export const setupMocks = () =>
	setupServer(
		...
		http.get(`${env.ENONICXP_SERVICES}/no.nav.navno/menu`, () => HttpResponse.json(testData)),
		http.get(`${env.APP_URL}/api/varselbjelle/varsler`, ({ request }) => {
			const notificationsState =
				localDecoratorMockState.notifications ?? getCookieValue(request, LOCAL_NOTIFICATIONS_COOKIE_NAME);

			return HttpResponse.json(
				notificationsState === 'empty' || (!notificationsState && process.env.MOCK_NOTIFICATIONS === 'empty')
					? { oppgaver: [], beskjeder: [] }
					: notificationsMock
			);
		}),
		http.get(`${env.APP_URL}/api/auth`, ({ request }) => {
			const authState = localDecoratorMockState.auth ?? getCookieValue(request, LOCAL_AUTH_COOKIE_NAME);
			const authLevel =
				authState === 'logged-in' ? '4' : authState === 'logged-out' ? undefined : process.env.MOCK_AUTH_LEVEL;
		...
		http.post(`${env.VARSEL_API_URL}/beskjed/inaktiver`, () => HttpResponse.json({ success: true })),
