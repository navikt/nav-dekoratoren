...
import { MessageIcon, TaskIcon } from 'decorator-shared/views/icons/notifications';
...
export type NotificationsService = {
    getNotifications: ({ texts, request }: { texts: Texts; request: Request }) => Promise<Notification[] | undefined>;
};
...
const getNotifications: (texts: Texts, data: NotificationData | null) => Promise<Notification[] | undefined> = async (texts, data) => {
    ...
    return Promise.resolve([...(data?.oppgaver.map(oppgaveToNotifiction) || []), ...(data?.beskjeder.map(beskjedToNotification) || [])]);
};
...
export const hentVarslerFetch = async (VARSEL_API_URL: string, request: Request): Promise<NotificationData | null> => {
...
export const getNotificationsProd = () => {
    return {
        getNotifications: async ({ request, texts }: { request: Request; texts: Texts }) => {
