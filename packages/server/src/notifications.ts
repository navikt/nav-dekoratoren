...
const varselSchema = z
	...
	.catch((ctx) => {
		...
		logger.error(`Error validating notification - ${JSON.stringify(redactedInput)}`);
		return null;
	});
...
const sortNewestFirst = (a: Varsel, b: Varsel) => (a.tidspunkt > b.tidspunkt ? -1 : 1);

const filterAndSort = (varsler: VarselNullable[]): Varsel[] =>
	varsler.filter((varsel): varsel is Varsel => !!varsel).sort(sortNewestFirst);

const varslerToNotifications = (varsler: Varsler): Notification[] =>
	[filterAndSort(varsler.oppgaver), filterAndSort(varsler.beskjeder)].flatMap((list) =>
		list.map((varsel): Notification => ({
			id: varsel.eventId,
			type: translateNotificationType[varsel.type] as NotificationType,
...
export const fetchNotifications = async ({ cookie }: { cookie: string }): Promise<ResultType<Notification[]>> => {
...
export const archiveNotification = async ({ cookie, id }: { cookie: string; id: string }) => {
