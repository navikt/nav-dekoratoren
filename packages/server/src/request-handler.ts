...
import RenderingService, { TemplateName } from './rendering-service';
...
const requestHandler = async (
    ...
    const handlers = new HandlerBuilder()
        ...
        .use(
            Object.entries(renderingService.templates).map(([name]) => {
                return {
                    method: 'POST',
                    ...
                    handler: async ({ request }) => {
                        ...
                        const { params } = data;
                        return htmlResponse(renderingService.render(name as TemplateName, params))
                    }
                }
            })
        )
