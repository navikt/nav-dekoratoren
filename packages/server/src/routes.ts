...
if (env.NODE_ENV === 'development' || isLocalhost()) {
	...
	setupMocks();
	routes.get('/mockServiceWorker.js', serveStatic({ path: './mockServiceWorker.js' }));
	routes.get(
		'/public/*',
		serveStatic({
			root: '../client/dist',
	...
	routes.get('/api/oauth2/session/refresh', async ({ req }) => fetch(req.url));
	...
	routes.get('/api/local-decorator-state', ({ req, json, redirect }) => {
		...
		setLocalDecoratorMockState({
			auth: auth === 'logged-in' || auth === 'logged-out' ? auth : undefined,
			notifications: notifications === 'full' || notifications === 'empty' ? notifications : undefined,
		});
...
routes.get('/csr/:clientWithId{client(.*).js}', async ({ redirect }) => redirect(csrAssets.csrScriptUrl));
routes.get('/csr/css/:clientWithId{client(.*).css}', async ({ redirect }) => redirect(csrAssets.cssUrl));
routes.get('/:clientWithId{client(.*).js}', async ({ redirect }) => redirect(csrAssets.csrScriptUrl));
routes.get('/css/:clientWithId{client(.*).css}', async ({ redirect }) => redirect(csrAssets.cssUrl));
