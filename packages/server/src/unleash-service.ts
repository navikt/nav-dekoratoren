...
export default class UnleashService {
    ...
    getFeatures(): Features {
        const features = Object.keys(this.defaultToggles).reduce((acc, toggle: string) => {
            // If mocking or running from localhost, return default toggles without
            // actually trying to get anything from Unleash Next
            if (this.mock) return { ...acc, [toggle]: this.defaultToggles[toggle] };
            ...
            const isFeatureEnabled = this.unleashInstance?.isSynchronized() ? this.unleashInstance.isEnabled(toggle) : this.defaultToggles[toggle];
