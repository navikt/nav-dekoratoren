	...
	return html`
		<consent-banner>
			<div class="${cls.container}">
				<section class="${cls.consentBanner}" aria-labelledby="consent_banner_title" id="consent-banner-dialog">
					<div class="${cls.miniContent}">
						${Button({
							content: html`<span>${i18n('consent_banner_minimized')}</span>${ExpandIcon({
					...
					<div class="${cls.content}">
						<h2 id="consent_banner_title" class="${cls.title}" tabindex="-1">${i18n('consent_banner_title')}</h2>
						...
						<p class="${cls.text}">${i18n('consent_banner_change_consent')}</p>
						<p class="${cls.text}">
							${i18n('consent_banner_additional_cookies_info')}${' '}
							<a href="${moreUrl}" class="${cls.moreLink}"> ${i18n('consent_banner_additional_cookies_link')} </a>
						</p>
					</div>
				</section>
			</div>
