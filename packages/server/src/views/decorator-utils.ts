...
import { AvailableLanguage, Breadcrumb, Language, UtilsBackground } from 'decorator-shared/params';
	...
	return html`
		<decorator-utils
			class="${clsx(cls.decoratorUtils, {
				[utils.hidden]: availableLanguages.length === 0 && breadcrumbs.length === 0,
				[cls.white]: utilsBackground === 'white',
				[cls.gray]: utilsBackground === 'gray',
			})}"
		>
			<div class="${clsx(cls.decoratorUtilsContent, utils.contentContainer)}">
				<d-breadcrumbs
					>${Breadcrumbs({
						breadcrumbs,
