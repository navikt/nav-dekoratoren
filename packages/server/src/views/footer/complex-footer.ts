...
export const ComplexFooter = ({ links, shareScreen }: { links: LinkGroup[]; shareScreen: boolean }) => html`
	<footer class="${cls.footer}" data-theme="dark">
		<div class="${cls.footerContent} ${utils.contentContainer}">
			<a
				class="${clsx(aksel['aksel-link'], aksel['aksel-body-short'], aksel['aksel-body-short--large'], cls.toTop)}"
				href="#toppen"
			...
			<ul class="${cls.footerLinks}">
				${links.map(
					({ heading, children }) => html`
						<li class="${cls.footerLinkGroup}">
							${
								heading &&
								html`<h2 class="${clsx(aksel['aksel-heading'], aksel['aksel-heading--xsmall'], cls.footerLinkHeading)}">
									${heading}
								</h2>`
							}
				...
				${shareScreen && html`<li>${ScreenshareButton(i18n('share_screen'))}</li>`}
			</ul>
			<div class="${cls.complexFooterOrg}">${NavLogo()}<span>Arbeids- og velferdsetaten</span></div>
		</div>
	</footer>
`;
