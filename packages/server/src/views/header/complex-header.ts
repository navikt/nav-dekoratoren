...
export function ComplexHeader({
	...
	return html`
		<div class="${cls.siteheader}" id="toppen" data-color="neutral">
			...
			${Sticky({
				children: html`
					<nav aria-label="${i18n('menu')}" class="${cls.hovedmenyWrapper} ${utilsCls.contentContainer}">
									...
									<context-links class="${cls.arbeidsflate}">
										${contextLinks.map(
											({ url, context }) => html`
												<a
													href="${url}"
													class="${clsx(cls.headerContextLink, context === currentContext && cls.lenkeActive)}"
						...
						<div class="${menuItemsCls.menuItems}" data-color="accent">
							...
							<div class="${menuItemsCls.menuItemsUniversalLinks}">
								${
									...
									DropdownMenu({
										...
										dropdownContent: html`
											<search-menu class="${menuCls.searchMenu}"> ${SearchForm()} </search-menu>
								...
								${DropdownMenu({
									...
									dropdownContent: html`
										<search-menu class="${menuItemsCls.searchMenu}" data-auto-focus> ${SearchForm()} </search-menu>
									`,
									attributes: {
										['menu-type']: 'search',
									},
