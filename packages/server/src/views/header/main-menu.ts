...
export function MainMenu({ title, frontPageUrl, links, contextLinks }: MainMenuProps) {
	return html`
		<div id="decorator-main-menu" class="${cls.mainMenu}">
			<div class="${cls.content}">
				<div class="${cls.header}">
					<h2 class="${clsx(aksel['aksel-heading'], aksel['aksel-heading--medium'])}">${title}</h2>
					<a href="${frontPageUrl}" class="${aksel['aksel-link']}"> ${i18n('to_front_page')} </a>
				</div>
				<div class="${cls.links}">
					${links.map(
						({ heading, children }) => html`
							<div class="${cls.linkGroup}">
								<h3 class="${clsx(aksel['aksel-heading'], aksel['aksel-heading--small'], cls.linkGroupHeading)}">
									${heading}
								</h3>
								<ul class="${cls.linkList}">
									${children.map(
										({ content, url }) => html`
											<li>
												<a href="${url}" class="${aksel['aksel-link']}" data-lenkegruppe="${heading}"> ${content} </a>
											</li>
										`
									)}
								</ul>
