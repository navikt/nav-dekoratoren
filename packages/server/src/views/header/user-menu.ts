...
import { BagdeIcon, LeaveIcon, PadlockLockedIcon, PersonCircleIcon } from 'decorator-icons';
	...
	<div class="${cls.userMenu}">
		<div class="${cls.menuItems}">
				...
				${
					...
					Alert({
						...
						content: html`
							...
							<a class="${aksel['aksel-link']}" data-color="neutral" href="${loginUrl}">
			...
			<a href="${personopplysningerUrl}" class="${cls.menuItem}">
				${level === 'Level4' ? BagdeIcon({ className: utils.icon }) : PadlockLockedIcon({ className: utils.icon })}
				<span>${i18n('personopplysninger')}</span>
			</a>
		</div>
		<div class="${cls.notifications}">${Notifications({ notifications, minsideUrl })}</div>
