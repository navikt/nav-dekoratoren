...
const renderLanguageOption = (lang: AvailableLanguage, currentLanguage: Language) => {
...
export const LanguageSelector = ({ availableLanguages, language }: LanguageSelectorProps) => html`
	<language-selector>
		<nav
		...
		>
			<button type="button" class="${cls.button}" aria-expanded="false" aria-controls="decorator-language-menu">
			...
			<ul class="${clsx(cls.menu, utils.hidden)}" id="decorator-language-menu">
				${availableLanguages.map((lang) => renderLanguageOption(lang, language))}
			</ul>
		</nav>
	</language-selector>
`;
