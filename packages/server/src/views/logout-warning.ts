...
export const LogoutWarning = () => html`
	<logout-warning>
		<token-dialog>
			<dialog class="${cls.modal}">
				<form class="${cls.modalWindow}">
					<h1 class="${cls.modalTitle}">${i18n('token_warning_title')}</h1>
					<p class="${cls.modalBody}">${i18n('token_warning_body')}</p>
		...
		<session-dialog>
			<dialog class="${cls.modal}">
				<form class="${cls.modalWindow}">
					<h1 class="${cls.modalTitle}">${i18n('session_warning_title')}</h1>
					<p class="${cls.modalBody}">${i18n('session_warning_body')}</p>
					<div class="${cls.buttonWrapper}">
						${Button({
							content: i18n('ok'),
							variant: 'primary',
