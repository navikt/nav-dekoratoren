...
import { MaskedNotification, Notification, UnmaskedNotification } from '../../notifications';
...
const MaskedNotificationComp = ({ notification: { type, date } }: { notification: MaskedNotification }) =>
	html` <div class="${cls.notification}">
		<div class="${cls.icon}">${type === 'task' ? ClipboardIcon() : ChatElipsisIcon()}</div>
		<div class="${cls.content}">
			<p class="${cls.text}">${type === 'task' ? i18n('masked_task_text') : i18n('masked_message_text')}</p>
			<p class="${cls.metadata}">
				<local-time datetime="${date}" class="${cls.date}"></local-time>
				•<span
					class="${clsx(aksel['aksel-tag'], aksel['aksel-tag--xsmall'])}"
...
const NotificationComp = ({
	...
	html` <link-notification class="${cls.notification}" data-type="${type}" data-id="${id}">
		<div class="${cls.icon}">${type === 'task' ? ClipboardIcon() : ChatElipsisIcon()}</div>
		<div class="${cls.content}">
			<div class="${cls.text}">
				<a href="${link}">${text}</a>
			...
			<div class="${cls.metadata}">
				<local-time datetime="${date}" class="${cls.date}"></local-time>
				${channels.length > 0 && html` <div class="">${kanalerToMetadata(channels)}</div>`}
				<div>${i18n(type)}</div>
			</div>
		</div>
	</link-notification>`;
		...
		<div class="${cls.content}">
			...
			<div class="${cls.metadata}">
				...
				${
					channels.length > 0 &&
					html` •
						<span class="channels">${kanalerToMetadata(channels)}</span>`
				}
				•
				<span
					class="${clsx(aksel['aksel-tag'], aksel['aksel-tag--xsmall'])}"
			...
			<button class="${clsx(aksel['aksel-button'], aksel['aksel-button--small'], cls.button)}" data-variant="secondary">
				${i18n('archive')}
			</button>
		</div>
	</archivable-notification>`;

export function Notifications({ notifications, minsideUrl }: NotificationsProps) {
									...
									<li>
										${
											notification.masked
												...
												: notification.type === 'message' && !notification.link
