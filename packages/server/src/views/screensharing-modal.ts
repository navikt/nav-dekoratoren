...
const ScreensharingModal = ({ children, status }: ScreensharingModalProps) => html`
	<screensharing-modal>
		<dialog class="${clsModal.modal} ${cls.screensharingModal}" data-status="${status}">
			...
			<div class="${clsModal.modalWindow} ${cls.content}">
				<h2 class="${clsModal.modalTitle}">${i18n('footer_del_skjerm')}</h2>
...
const ScreensharingEnabled = () => {
	return ScreensharingModal({
		...
		children: html`
			<form>
				<div>
					<label for="screensharing_code" class="${clsInputs.textInputLabel}">${i18n('delskjerm_modal_label')}</label>
					<input
