...
const getScriptsProps = async (): Promise<HtmlElementProps[]> => {
	...
	const manifest = (await import('decorator-client/dist/.vite/manifest.json')).default as ViteManifest;
...
const scriptsHtml = unsafeHtml(scriptsProps.map(buildHtmlElementString).join(''));
...
export const ScriptsTemplate = (props: DecoratorDataProps): Template => {
	return html`
		...
		<script id="d-data-parser">
			window.__DECORATOR_DATA__ = JSON.parse(document.getElementById('__DECORATOR_DATA__')?.textContent ?? '');
		</script>
