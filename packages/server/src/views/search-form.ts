	...
	return html`<form class="${cls.searchForm}">
		...
		<div class="${cls.searchWrapper}">
			<search-input class="${cls.searchWrapperInner}">
				<input class="${cls.searchInput}" type="text" name="search" maxlength="100" id="${id}" autocomplete="off" />
				<button type="button" class="${cls.clear} ${utils.hidden}">${XMarkIcon({ ariaLabel: i18n('clear') })}</button>
			</search-input>
