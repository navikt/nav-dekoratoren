...
export const SearchHits = ({ results: { hits, total }, query, texts }: SearchHitsProps) => html`
    <div class="${cls.searchHits}">
        ${total === 0
            ...
            : html`<ul class="${cls.searchHitList}">
                  ...
                  <div>
                      <div role="status">${texts.showing} ${Math.min(total, 5).toString()} ${texts.of} ${total.toString()} ${texts.results}</div>
                      <a href="https://www.nav.no/sok?ord=${query}"> ${texts.see_all_hits} ("${query}") </a>
                  </div>`}
    </div>
`;
