...
export const test = (name: string, fn: (args: { page: Page }) => Promise<void>) => {
	[
		{ test: csr, name: 'csr' },
		{ test: ssr, name: 'ssr' },
		{ test: nextPagesRouter, name: 'next pages router' },
	].forEach(({ test, name: fixtureName }) => test(`${fixtureName}: ${name}`, fn));
};
...
export const testWithUmamiMock = (name: string, fn: (args: { page: Page }) => Promise<void>) => {
	[
...
export const waitForDecoratorReady = (page: Page) => page.evaluate(decoratorReady);
