...
const getSsrDecoratorParams = (html: string) => {
	const scriptStart = '<script type="application/json" id="__DECORATOR_DATA__">';
...
testNextPagesRouter('sets route-derived params during SSR', async ({ page }) => {
	const response = await page.request.get(`${nextPagesRouterUrl}/arbeidsgiver?language=en`);
...
testNextPagesRouter('updates params from client navigation', async ({ page }) => {
	...
	await page.getByRole('button', { name: 'Legg til' }).click();
	await page.getByLabel('Context', { exact: true }).selectOption('arbeidsgiver');
	await page.getByLabel('Rute', { exact: true }).selectOption('arbeidsgiver');
	await page.getByRole('button', { name: 'Naviger med Next Link' }).click();

	await expect(page).toHaveURL(`${nextPagesRouterUrl}/arbeidsgiver?language=en&context=arbeidsgiver`);
	await expect
	...
	await page.getByRole('button', { name: 'Naviger med router.replace' }).click();

	await expect(page).toHaveURL(`${nextPagesRouterUrl}/person?simple=true`);
	await expect
		.poll(() => getDecoratorParams(page))
		.toMatchObject({
...
testNextPagesRouter('sets local auth and notification state', async ({ page }) => {
	await page.getByRole('button', { name: 'Logg ut' }).click();

	await expect(page.getByRole('link', { name: 'Logg inn', exact: true })).toBeVisible();

	await page.getByRole('button', { name: 'Logg inn uten varsler' }).click();

	await expect(page.getByRole('button', { name: 'Ambisiøs Utsikt' })).toBeVisible();
...
testNextPagesRouter('updates and resets params directly', async ({ page }) => {
	await expect(page.getByRole('button', { name: 'Fjern alle' })).toBeDisabled();
	...
	expect(await getDecoratorParams(page)).toMatchObject({
		breadcrumbs: expect.arrayContaining([expect.objectContaining({ title: 'Sykepenger' })]),
	});

	await page.getByLabel('Språk', { exact: true }).selectOption('nb');
	await page.getByLabel('Brødsmuler', { exact: true }).selectOption('application');
	await page.getByLabel('Vis chatbot', { exact: true }).uncheck();
	await page.getByRole('button', { name: 'Oppdater via window.postMessage' }).click();
	...
	await expect
		...
		.toMatchObject({
			...
			breadcrumbs: expect.arrayContaining([expect.objectContaining({ title: 'Søknad om sykepenger' })]),
			pageType: 'direkte-test',
		});
